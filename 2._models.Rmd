...
Traditionally, the approach to quantifying the relation between smoking and high blood pressure is to identify a set of confounders. Let's assume for our purpose that the relevant confounders are: `sex, age, race, income, marital, school,` and `birthcontrol`. The most common approach to estimating the effect of smoking on high blood pressure is to fit a logistic model, adjusting for these confounders:

```{r}
model1 <- glm(hbp~qsmk+sex+age+race+income+marital+school+birthcontrol+diabetes,data=a,family=binomial(link="logit"))
summary(model1)
```
The output from this model tells us that the odds of HBP among smokers is `r round(exp(summary(model1)$coefficients[2,1]),2)` times the odds of HBP among non-smokers (with 95% CIs of `r round(exp(summary(model1)$coefficients[2,1] - 1.96*summary(model1)$coefficients[2,2] ),2)`, `r round(exp(summary(model1)$coefficients[2,1] + 1.96*summary(model1)$coefficients[2,2] ),2)`).

It might be tempting to conclude that this is the effect of smoking on high blood pressure. One might be further tempted to interpret this effect as a comparison of the odds of high blood pressure if everyone smoked versus if no one smoked. Unfortunately, there are a number of considerations related to the types of models we are using that jeapordize the validity of such an interpretation.
...
These relations are codified in the causal model, but not the staitstical model. For example, depending on how/when it was measured, we might draw a directed acyclic graph in which diabetes is a common cause of both smoking (becuase diabetics will refrain from smoking) and high blood pressure, or in which diabetes mediates (smoking causes diabetes) the relation between smoking and high blood pressure.  
...
The critical point is that if the interest if the total effect of smoking on the outcome is of interest, and if diabetes is in fact a mediator of this relation, then it should **not** be adjusted for.^[Note that at times, clinicians/researchers are primarily interested in the effect of smoking independent of it's effect on diabetes. In this case, methods for mediation analysis are required.] Otherwise, a key part of the effect will be blocked. Alternatively, if diabetes is confounding the relation between smoking and high blood pressure, then it must be adjusted for to reduce confounding bias.
...
A third complication with interpreting the estimate for smoking as "what would be observed if everyone smoked versus if no one smoked" is the distinction between marginal and conditional models. This distinction becomes more complicated when interest lies in estimating non-collapsible parameters (i.e., non-linear model) such as the odds ratio or the hazard ratio [@Greenland2005b].
...
For a linear model, the marginal and conditional effect estimate will be equivalent, unless there is an interaction between the exposure and a covariate (in which case they may, but need not be, equivalent). For example, if we consider the relation between smoking and high blood pressure after adjusting for sex, we can obtain a conditional and marginal risk difference as follows:
```{r}

linear_model <- glm(hbp ~ qsmk + sex, data=a, family=gaussian("identity"))
round(coef(linear_model)[2]*100,2)

```
Thus the conditional risk difference is `r round(coef(linear_model)[2]*100,2)` excess cases of high blood pressure among smokers versus nonsmokers per 100 participants. We can marginalize over the distribution of sex in the sample as:
...
```
Thus the marginal risk difference is `r round(mean(risk1 - risk0)*100,2)` excess cases of high blood pressure among smokers versus nonsmokers per 100 participants, the same as the conditional risk difference.

If we interact smoking and sex, the conditional and marginal are no longer equivalent because there are two effects for the conditional model (one for each smoking level), but only one for the marginal. 
```{r}

linear_model2 <- glm(hbp ~ qsmk + sex + qsmk:sex, data=a, family=gaussian("identity"))
# effect among sex = 0
...
logit_model <- glm(hbp ~ qsmk + sex, data=a, family=binomial("logit"))
